classdef normal_normal_wishart3 < handle
    ...
    methods
        function self=normal_normal_wishart3(ndim,nstates)
            ...
            elseif strcmp(opttrain,'VB') %Used for Variational Bayes   
                for state=k1:k2
                    ...
                    if Nk < 15 % extinguished 
                        ...
                        self.posterior.mean_normal{state}.prec_gamma.scale=(1./b0+1/2*((mk-m0).^2+diag(inv(Rk))')).^-1;
                    end
                end
            end
        end
        ...
        function priornoinf(self,type,X)
            ...
            if strcmp(type,'default')
                ...
                self.prior.mean_normal{1}.prec_gamma.shape=ones(1,n)*0.001;
                self.prior.mean_normal{1}.prec_gamma.scale=ones(1,n)*1000;
        ...
        function []=divklfun(self)
            ...
            for state=1:self.nstates
                ...
                Wk=self.posterior.prec_wishart{state}.scale;
                if size(self.prior.mean_normal,2)>1
                    %R0=self.prior.mean_normal{state}.prec_gamma.shape*self.prior.mean_normal{state}.prec_gamma.shape;
                ...
                for j=1:self.ndim         
                    D=D+util.Gamma.klgamma(ak(j),1/bk(j),a0(j),1/b0(j));   
                end              
