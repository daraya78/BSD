classdef normal_normal_wishart2 < handle
    ...
    methods
        function self=normal_normal_wishart2(ndim,nstates)
            ...
            self.prior.mean_normal{1}.prec_gamma.shape=[];
            self.prior.mean_normal{1}.prec_gamma.scale=[];
            self.prior.prec_wishart{1}.degree=[];
            self.prior.prec_wishart{1}.scale=[];
            self.posterior.mean_normal{1}.mean=[];
            self.posterior.mean_normal{1}.prec_gamma.shape=[];
            self.posterior.mean_normal{1}.prec_gamma.scale=[];
        ...
        function update (self,opttrain,X,gammain,statereq)
            ...
            elseif strcmp(opttrain,'VB') %Used for Variational Bayes   
                for state=k1:k2
                    ...
                    a0=self.prior.mean_normal{nprior}.prec_gamma.shape;
                    b0=self.prior.mean_normal{nprior}.prec_gamma.scale;
                    ...
                    if length(self.posterior.mean_normal)==self.nstates  
                        ak=self.posterior.mean_normal{state}.prec_gamma.shape;
                        bk=self.posterior.mean_normal{state}.prec_gamma.scale;
                        ...
                        ak=self.prior.mean_normal{1}.prec_gamma.shape;
                        bk=self.prior.mean_normal{1}.prec_gamma.scale;
                    ...
                    if Nk < 15 % extinguished 
                        ...
                        Rk=ak*bk*eye(self.ndim)+Nk*(vk*Wk);
                        self.posterior.mean_normal{state}.prec=Rk;
                        self.posterior.mean_normal{state}.mean=(m0*diag(ak.*bk)+xbar*Nk*(vk*Wk))*(self.posterior.mean_normal{state}.prec)^-1;
                        mk=self.posterior.mean_normal{state}.mean;
                        self.posterior.mean_normal{state}.prec_gamma.shape=a0+1/2;
                        self.posterior.mean_normal{state}.prec_gamma.scale=(1./b0+1/2*((mk-m0)*(mk-m0)'+trace(inv(Rk)))).^-1;
                    end
                end
            end
        end
        ...
        function priornoinf(self,type,X)
            ...
            if strcmp(type,'default')
                ...
                self.prior.mean_normal{1}.prec_gamma.shape=0.001;
                self.prior.mean_normal{1}.prec_gamma.scale=1000;
        ...
        function []=divklfun(self)
            ...
            for state=1:self.nstates
                ...
                Wk=self.posterior.prec_wishart{state}.scale;
                if size(self.prior.mean_normal,2)>1
                    a0=self.prior.mean_normal{state}.prec_gamma.shape;
                    b0=self.prior.mean_normal{state}.prec_gamma.scale;
                    R0=ak.*bk*eye(self.ndim);
                    m0=self.prior.mean_normal{state}.mean;
                    v0=self.prior.prec_wishart{state}.degree;
                    W0=self.prior.prec_wishart{state}.scale;
                else
                    a0=self.prior.mean_normal{1}.prec_gamma.shape;
                    b0=self.prior.mean_normal{1}.prec_gamma.scale;
                    R0=ak.*bk*eye(self.ndim);
                    m0=self.prior.mean_normal{1}.mean;
                    v0=self.prior.prec_wishart{1}.degree;
                    W0=self.prior.prec_wishart{1}.scale;
                end
                D=D+util.Gamma.klgamma(ak,1/bk,a0,1/b0);   
